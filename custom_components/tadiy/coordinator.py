...
class TaDIYDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        self.heat_models: dict[str, HeatUpModel] = {}
        ...
        # Schedule engine
        self.schedule_engine = ScheduleEngine(frost_protection_temp=frost_protection_temp)

        # Storage
        self.learning_store = Store(hass, STORAGE_VERSION, f"{STORAGE_KEY}_{entry_id}")
        ...
        for room_data in rooms:
            try:
                ...
                if not isinstance(room_data, dict):
                    _LOGGER.error("Invalid room data type: %s", type(room_data))
                    continue

                room_name = room_data.get("name")
                if not room_name:
                    _LOGGER.error("Room data missing 'name' field: %s", room_data)
                    continue

                room = RoomConfig.from_dict(room_data)
                self.rooms.append(room)
                self.heat_models[room.name] = HeatUpModel(room_name=room.name)
                _LOGGER.debug("Loaded room configuration: %s", room.name)
            except (ValueError, KeyError) as err:
                room_name = room_data.get("name", "Unknown")
                _LOGGER.error("Failed to load room %s: %s", room_name, err, exc_info=True)
        ...
        for room_name, model_data in data.items():
            if room_name in self.heat_models:
                try:
                    ...
                    _LOGGER.warning("Failed to load learning data for %s: %s", room_name, err)

    async def async_save_learning_data(self) -> None:
        """Save learning data to storage."""
        data = {name: model.to_dict() for name, model in self.heat_models.items()}
        await self.learning_store.async_save(data)
    ...
    async def async_load_schedules(self) -> None:
        ...
        self.schedule_engine.set_frost_protection_temp(self.frost_protection_temp)
        ...
        for room_name, schedule_data in rooms_data.items():
            try:
                ...
                _LOGGER.warning("Failed to load schedule for %s: %s", room_name, err)
    ...
    def get_frost_protection_temp(self) -> float:
        ...
        if number_state and number_state.state not in ("unknown", "unavailable"):
            try:
                self.frost_protection_temp = float(number_state.state)
    ...
    def get_override_info(self, room_name: str) -> dict[str, Any]:
        ...
        return self.overrides.get(room_name, {"active": False, "until": None})
    ...
    async def _fetch_room_data(
        ...
        heat_model = self.heat_models.get(room_config.name)
